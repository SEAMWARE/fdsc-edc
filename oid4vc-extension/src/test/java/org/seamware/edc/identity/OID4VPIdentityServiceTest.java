...
public class OID4VPIdentityServiceTest {
    ...
    public void setup() {
        ...
        oid4VPIdentityService = new OID4VPIdentityService(mock(Monitor.class), tokenProvider, TEST_CLIENT_ID, TEST_SCOPE);
    }

    @Test
    public void testObtainClientCredentials_success() {
        when(tokenProvider.getAccessToken(any())).thenReturn(CompletableFuture.completedFuture(getTokenResponse(TEST_TOKEN)));

        TokenRepresentation expectedTokenRep = getTokenRepresentation(TEST_TOKEN);

        Result<TokenRepresentation> tokenResult = oid4VPIdentityService.obtainClientCredentials(getTokenParameters());
        assertTrue(tokenResult.succeeded(), "A token should have been obtained successfully.");


        assertEquals(expectedTokenRep.getToken(), tokenResult.getContent().getToken(), "The expected token should have been returned");
        assertEquals(expectedTokenRep.getExpiresIn(), tokenResult.getContent().getExpiresIn(), "The expected token should have been returned");
        assertEquals(expectedTokenRep.getAdditional(), tokenResult.getContent().getAdditional(), "The expected token should have been returned");
    }
    ...
    public void testObtainClientCredentials_incomplete_token(String name, TokenResponse invalidResponse) {
        when(tokenProvider.getAccessToken(any())).thenReturn(CompletableFuture.completedFuture(invalidResponse));
    ...
    private static Stream<Arguments> getInvalidTokens() {
        return Stream.of(
                ...
                Arguments.of("TokenResponse needs to contain expiry.", new TokenResponse().setAccessToken(TEST_TOKEN)),
                Arguments.of("TokenResponse needs to contain token.", new TokenResponse().setExpiresIn(10l)),
                Arguments.of("TokenResponse needs to contain non empty token.", new TokenResponse().setAccessToken("").setExpiresIn(10l)),
                Arguments.of("TokenResponse needs to contain a valid expiry.", new TokenResponse().setAccessToken(TEST_TOKEN).setExpiresIn(0l))
        );
    }
    ...
    public void testVerifyJwtToken_success() {
        ...
        Result<ClaimToken> claimTokenResult = oid4VPIdentityService.verifyJwtToken(tokenRepresentation, null);
        assertTrue(claimTokenResult.succeeded(), "The claim token should have been extracted.");
        assertEquals(expectedClaimToken.getClaims(), claimTokenResult.getContent().getClaims(), "All claims should be included.");

    }

    @ParameterizedTest(name = "Invalid token - {0}")
    @MethodSource("getInvalidTokenRepresentation")
    public void testVerifyJwtToken_invalid_token(String name, TokenRepresentation invalidRepresentation) {
        Result<ClaimToken> claimTokenResult = oid4VPIdentityService.verifyJwtToken(invalidRepresentation, null);
    ...
    private static Stream<Arguments> getInvalidTokenRepresentation() {
        return Stream.of(
                ...
                Arguments.of("TokenRepresentations without an token are invalid.", TokenRepresentation.Builder.newInstance().build()),
                Arguments.of("Empty accesstokens are invalid.", getTokenRepresentation("")),
                Arguments.of("Only valid JWTs can be used as tokens.", getTokenRepresentation("something-wild"))
        );
    }
