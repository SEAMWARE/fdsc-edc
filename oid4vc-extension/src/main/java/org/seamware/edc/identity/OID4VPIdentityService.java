...
/**
 * Handle identity in an OID4VC based security ecosystem. 1. Authenticate via OID4VP 2. Accept and
 * decode JWTs already verified by an apigateways
 */
public class OID4VPIdentityService implements org.eclipse.edc.spi.iam.IdentityService {
  ...
  public OID4VPIdentityService(
      Monitor monitor, TokenProvider tokenProvider, String clientId, Set<String> scope) {
    ...
    try {
      ...
      RequestParameters requestParameters =
          new RequestParameters(
              URI.create(audURI.getScheme() + "://" + audURI.getAuthority()), "", clientId, scope);
  ...
  private boolean validTokenResponse(TokenResponse tokenResponse) {
    return tokenResponse.getAccessToken() != null
        ...
        && tokenResponse.getExpiresIn() > 0;
  }

  /**
   * In case of the FIWARE Dataspace Connector, the EDC endpoints are ALWAYS protected by the
   * PEP(e.g. Apisix) which is responsible for verifying the token. The method only has to do the
   * decoding.
   */
  @Override
  public Result<ClaimToken> verifyJwtToken(
    ...
    try {
      ...
      return Result.failure(
          "[OID4VPIdentityService] Was not able to read the token " + e.getMessage() + " '");
    }
  }
}
