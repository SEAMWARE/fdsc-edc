...
public class OID4VPConfig {
    ...
    // folder containing additional trust anchors for X509_SAN_DNS resolution. If empty or not configured, the system truststore is used
    ...
    public static OID4VPConfig fromConfig(Config config) {
        ...
        getNullSafeFromConfig(() -> oid4VpConfig.getBoolean("enabled")).ifPresent(configBuilder::enabled);
        getNullSafeFromConfig(() -> oid4VpConfig.getString("clientId")).ifPresent(configBuilder::clientId);
        getNullSafeFromConfig(() -> oid4VpConfig.getBoolean("trustAll")).ifPresent(configBuilder::trustAll);
        ...
        getNullSafeFromConfig(() -> oid4VpConfig.getString("credentialsFolder")).ifPresent(configBuilder::credentialsFolder);
        getNullSafeFromConfig(() -> oid4VpConfig.getString("trustAnchorsFolder")).ifPresent(configBuilder::trustAnchorsFolder);
        getNullSafeFromConfig(() -> oid4VpConfig.getString("organizationClaim")).ifPresent(configBuilder::organizationClaim);
        ...
        getNullSafeFromConfig(() -> holderConfig.getString("signatureAlgorithm")).ifPresent(holderConfigBuilder::signatureAlgorithm);
        ...
        getNullSafeFromConfig(() -> proxyConfig.getBoolean("enabled")).ifPresent(proxyConfigBuilder::enabled);
    ...
    public record ProxyConfig(boolean enabled, String host, int port) {
        public static class Builder {
            ...
            public ProxyConfig build() {
                if (enabled) {
                    Objects.requireNonNull(host, "If a proxy should be used for OID4VP, the proxy host needs to be configured.");
                }
    ...
    public static class Builder {
        ...
        public OID4VPConfig build() {
            if (oid4VPConfig.enabled) {
                ...
                Objects.requireNonNull(oid4VPConfig.scope, "When OID4VP is enabled, a scope has to be provided.");
            }
            return oid4VPConfig;
        }
    }
