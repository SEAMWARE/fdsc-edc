...
public class TransferConfig {
  ...
  public static TransferConfig fromConfig(Config config) {
    ...
    getNullSafeFromConfig(() -> transferConfig.getBoolean("enabled"))
    ...
    getNullSafeFromConfig(() -> oid4vcConfig.getBoolean("enabled"))
        .ifPresent(oid4VcBuilder::enabled);
    getNullSafeFromConfig(() -> oid4vcConfig.getString("verifierHost"))
        .ifPresent(oid4VcBuilder::verifierHost);
    getNullSafeFromConfig(() -> oid4vcConfig.getString("odrlPapHost"))
        .ifPresent(oid4VcBuilder::odrlPapHost);
    getNullSafeFromConfig(() -> oid4vcConfig.getString("verifierInternalHost"))
        .ifPresent(oid4VcBuilder::verifierInternalHost);
    getNullSafeFromConfig(() -> oid4vcConfig.getString("opaHost"))
        .ifPresent(oid4VcBuilder::opaHost);
    getNullSafeFromConfig(() -> oid4vcConfig.getString("credentialsConfigAddress"))
        .ifPresent(oid4VcBuilder::credentialsConfigAddress);
    ...
    getNullSafeFromConfig(() -> dcpOidConfig.getString("openIdPath"))
        .ifPresent(oidConfigBuilder::openIdPath);
    getNullSafeFromConfig(() -> dcpOidConfig.getString("jwksPath"))
        .ifPresent(oidConfigBuilder::jwksPath);
    ...
    getNullSafeFromConfig(() -> apisixConfig.getString("address"))
        .ifPresent(apisixBuilder::address);
   ...
   * @param httpsProxy address of an httpsProxy to be added to the routes. If null, no proxy will be
   *     used
   */
    ...
    public static class Builder {
      ...
      public Apisix build() {
        Objects.requireNonNull(
            address, "If FDSC Transfer is enabled, an apisix address needs to be configured.");
        Objects.requireNonNull(
            token, "If FDSC Transfer is enabled, an apisix admin token needs to be configured.");
   ...
   * @param credentialsConfigAddress - address of the credentialsConfigService to be used for
   ...
   */
  public record Oid4Vc(
      boolean enabled,
      String credentialsConfigAddress,
      String verifierHost,
      String verifierInternalHost,
      String opaHost,
      String odrlPapHost) {
  ...
  public record Dcp(boolean enabled, OidConfig oidConfig) {
    ...
    public static class Builder {
      ...
      public Dcp build() {
        if (enabled) {
          Objects.requireNonNull(
              oidConfigBuilder, "If DCP is enabled, the OID endpoints have to be configured.");
  ...
  public static class Builder {
    ...
    public TransferConfig build() {
      if (transferConfig.enabled) {
        Objects.requireNonNull(
            transferConfig.getApisix(),
            "If FDSC Transfers are supported, the Apisix Admin access needs to be provided.");
        Objects.requireNonNull(
        ...
        if (transferConfig.dcp == null) {
          transferConfig.dcp = new Dcp.Builder().enabled(false).build();
        }
      }
