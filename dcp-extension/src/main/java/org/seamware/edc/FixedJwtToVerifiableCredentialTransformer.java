...
public class FixedJwtToVerifiableCredentialTransformer
    extends JwtToVerifiableCredentialTransformer {
  ...
  @Override
  public @Nullable VerifiableCredential transform(
      @NotNull String serializedJwt, @NotNull TransformerContext context) {
    try {
      ...
      if (isVcDataModel2_0(claims)) {
        vcObject =
            claims.getClaims(); // in VCDM2.0 the credential is directly stored in the payload
      ...
      if (vcObject instanceof Map<?, ?> vc) {
        ...
        // credential subjects
        listOrReturn(
                vc.get(CREDENTIAL_SUBJECT_PROPERTY),
                o -> extractSubject((Map<String, ?>) o, claims.getSubject()))
            .forEach(builder::credentialSubject);

        // credential status
        listOrReturn(
                vc.get(CREDENTIAL_STATUS_PROPERTY), o -> extractStatus((Map<String, Object>) o))
            .forEach(builder::credentialStatus);

        // credential schema
        listOrReturn(
                vc.get(CREDENTIAL_SCHEMA_PROPERTY), o -> extractSchema((Map<String, Object>) o))
            .forEach(builder::credentialSchema);

        // expiration date
        extractDate(vc.get(EXPIRATION_DATE_PROPERTY), claims.getExpirationTime())
            .or(() -> extractDate(vc.get(VALID_UNTIL_PROPERTY), claims.getExpirationTime()))
        ...
        var issuer =
            ofNullable(claims.getIssuer())
                .or(() -> ofNullable(vc.get("issuer")).map(Object::toString))
                .orElse(null);
   ...
   *
   * TODO: check with upcoming Keycloak changes. Might be viewed as a bug and changed.
   */
  private Optional<Instant> extractDate(@Nullable Object dateObject, Date fallback) {
    try {
      ...
      if (dateObject instanceof Double numeric) {
        // small loss of precision is likely due to double conversion already, should not have any
        // practical impact.
