...
public class CatalogProtocolServiceExtension implements ServiceExtension {
    ...
    public void initialize(ServiceExtensionContext context) {
        ...
        if (tmfConfig.isEnabled() && tmfConfig.getCatalogConfig().enabled()) {
            context.registerService(CatalogProtocolService.class, new TMForumBackedCatalogProtocolService(tmfEdcMapper, productCatalogApi, context.getParticipantId(), monitor, protocolTokenValidator));
        } else {
